...
export function AudioSection() {
  ...
  return (
    <section ref={ref} className="relative bg-[var(--section-alt-bg)] text-[var(--section-alt-fg)] transition-colors duration-500">
      {/* Subtle grid */}
      <div
        className="absolute inset-0 opacity-[0.05] pointer-events-none"
        style={{
          backgroundImage: "radial-gradient(var(--text-3) 0.5px, transparent 0.5px)",
          backgroundSize: "40px 40px"
        }}
      />

      <div className="relative mx-auto grid max-w-content gap-10 px-5 py-24 lg:grid-cols-[0.8fr_1.2fr]">
        {/* Sticky sidebar */}
        <aside className="top-28 self-start lg:sticky">
          ...
          <h2 className="mt-4 text-4xl font-bold leading-tight text-[var(--section-alt-fg)] md:text-5xl">
            Four models.<br />One interface.
          </h2>
          <div className="mt-10 flex gap-5">
            <ProgressLine progress={progress} />
            <p className="max-w-xs text-sm leading-6 text-[var(--section-alt-fg-2)]">
          ...
          <div className="mt-8 grid grid-cols-2 gap-3">
            {[["8", "Tools"], ["4", "AI Models"], ["99+", "Languages"], ["0", "Cloud upload"]].map(([val, label]) => (
              <div key={label} className="rounded-xl bg-[var(--surface)] border border-[var(--border)] p-3 shadow-sm">
                <span className="text-2xl font-bold text-[var(--section-alt-fg)]">{val}</span>
                <p className="text-xs text-[var(--section-alt-fg-2)] mt-0.5">{label}</p>
              </div>
            ))}
          </div>
        </aside>
        ...
        <div className="grid gap-3">
          {tools.map(([model, name, description, color], index) => (
            <motion.article
              ...
              className="group relative rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 hover:border-accent/30 hover:shadow-lg transition-all duration-300 card-lift cursor-default"
              key={name}
            >
              {/* Left accent bar */}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 pl-4">
                  <Badge>{model}</Badge>
                  <h3 className="mt-4 text-lg font-semibold text-[var(--section-alt-fg)]">{name}</h3>
                  <p className="mt-2 text-sm leading-6 text-[var(--section-alt-fg-2)]">{description}</p>
                </div>

                {/* Mini waveform */}
                <div className="ml-4 hidden sm:flex items-end gap-0.5 h-8 shrink-0">
                  {[...Array(8)].map((_, i) => (
              ...
              <div className="mt-4 flex items-center justify-between pl-4 text-sm">
                ...
                <Link href="/audio" className="text-[var(--section-alt-fg-2)] hover:text-accent transition-colors duration-200 text-sm font-medium hover:underline underline-offset-2">
                  Open Tool →
                </Link>
              </div>
            </motion.article>
          ...
          >
            <div className="rounded-xl bg-[var(--surface-2)] p-5 border border-[var(--border)]">
              <div className="flex items-center justify-between text-xs text-[var(--section-alt-fg-2)] mb-1">
              ...
              <div className="grid gap-2">
                {waveWidths.map((width, index) => (
                  <div className="flex items-center gap-2" key={width + index}>
                    ...
                    <div className="flex-1 h-7 rounded-lg bg-[var(--surface)] border border-[var(--border)] overflow-hidden">
                      <motion.div
